...
3. Query the insight index for terms from the requirement — an entry about a stack quirk or a non-obvious truth may constrain your design (e.g. "Vendor SDK v2.7.1 returns null for invalid keys" affects error-handling design). You hold no `Bash`, so use `Grep` with an explicit `path`; see `.harness/rules/05-insight-index.md` for why an unscoped search returns nothing.
