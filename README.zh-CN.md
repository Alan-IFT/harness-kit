...
> **Claude Code 的 Harness Engineering 工具包** — 一个 Claude Code Plugin（9 个 skills + 项目模板），把"有纪律的 AI 驱动开发"带到全栈和后端项目里。
>
> **目标**：人工只做"描述需求"和"AI 做不到时介入"；其他全部 — 7-Agent 流水线、验证闸门、结构化文档 — 自动运行。

## 包含什么

这是一个 Claude Code Plugin 包，给任何项目装上 9 个 AI skill：
...
- `verify_all`（27 项检查）— 仓库本身健康度
- `test-init`（177 断言）— init 模板逻辑（空目录）
...
| 0.16+ | 规划中 | 真正的 **AI 原生 init**（AI 读项目描述 + 已有代码后生成自定义 50-*.md 和分区 agent）；supervisor agent 旁观流水线进度 |
