...
- `test-init`（PowerShell 275 断言；不带 python3 的 Bash 237）— init 模板逻辑（空目录；覆盖 3 种项目类型，外加 migrate-layout 块、zh-overlay 消费者分流策略断言、以及 BUG-2 placeholder 正则回归（实时计数见 `.harness/scripts/baseline.json`）
