...
- `.claude/`（Claude Code binding：`agents/` + `skills/` 从 `.harness/` 同步，外加 `settings.json`）
...
- `test-init`（PowerShell 255 断言；不带 python3 的 Bash 217）— init 模板逻辑（空目录；3 种项目类型 × 75 PS / 63 Bash，外加 migrate-layout 块、4 条不依赖 shell 的 zh-overlay 消费者分流策略断言、以及 2 条不依赖 shell 的 BUG-2 placeholder 正则回归断言）
...
本仓库自己用 Harness Kit 开发。同样的 7-agent 流水线既给用户也管理本仓库的工作，跑在同一份 `.harness/` 真相源（agents / rules / skills）上 —— 就是 init 写进新项目的那份。如果我们自己用不下去，就不该 ship。
...
3. **工具无关 SOT vs binding 层** — `.harness/` 是真相；`.claude/agents/` + `.claude/skills/` 是同步出来的 binding；`CLAUDE.md` + `.github/copilot-instructions.md` 是静态 bootstrap stub
