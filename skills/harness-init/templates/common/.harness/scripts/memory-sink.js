...
exports.SqliteSink = exports.MemoryDbPathError = exports.DEFAULT_TOKEN_BUDGET = exports.DEFAULT_LIMIT = void 0;
...
exports.DEFAULT_TOKEN_BUDGET = 2000;
function loadSqlite() {
...
// ── db location ──────────────────────────────────────────────────────────────
class MemoryDbPathError extends Error {
}
exports.MemoryDbPathError = MemoryDbPathError;
function resolveDbPath(root, env = process.env) {
    const explicit = env['HARNESS_MEMORY_DB'];
    const data = env['CLAUDE_PLUGIN_DATA'];
    const chosen = explicit !== undefined && explicit !== ''
    ...
    if (pluginRoot !== undefined && pluginRoot !== '') {
        ...
        if (rel !== '' && !rel.startsWith('..') && !path.isAbsolute(rel)) {
            throw new MemoryDbPathError(`refusing to put memory.db inside CLAUDE_PLUGIN_ROOT (${pr}) — that directory is ` +
...
class SqliteSink {
    ...
    migrate() {
        ...
        this.db.exec(SCHEMA);
        this.db.exec(`INSERT INTO records_fts (rowid, title, body, tags) SELECT rowid, title, body, tags FROM records;`);
    ...
    upsert(records) {
        ...
        let updated = 0;
        let skipped = 0;
        const existing = this.db.prepare('SELECT rowid, seq, content_hash FROM records WHERE id = ? ORDER BY seq DESC');
        const insert = this.db.prepare(`INSERT INTO records (id, seq, kind, title, body, date, source_task, evidence, tags, content_hash, supersedes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        const update = this.db.prepare(`UPDATE records SET kind = ?, title = ?, body = ?, date = ?, source_task = ?, evidence = ?,
        ...
        try {
            for (const rec of records) {
                ...
                if (sameSeq !== undefined) {
                    update.run(rec.kind, rec.title, rec.body, rec.date, rec.sourceTask, JSON.stringify(rec.evidence), JSON.stringify(rec.tags), rec.contentHash, rec.supersedes ?? null, Number(sameSeq['rowid']));
                    updated += 1;
                    continue;
                }
                ...
                const prior = rows[0];
                const supersedes = rec.supersedes ?? (prior !== undefined && Number(prior['seq']) < rec.seq ? `${rec.id}@${Number(prior['seq'])}` : null);
                insert.run(rec.id, rec.seq, rec.kind, rec.title, rec.body, rec.date, rec.sourceTask, JSON.stringify(rec.evidence), JSON.stringify(rec.tags), rec.contentHash, supersedes);
                inserted += 1;
            }
            this.db.exec('COMMIT');
        }
    ...
    search(query, opts = {}) {
        ...
        const budget = opts.tokenBudget ?? exports.DEFAULT_TOKEN_BUDGET;
        const match = toMatchQuery(query);
        if (match === '')
            return { records: [], tokensUsed: 0, tokensBudget: budget, truncated: false };
        ...
        const sql = `SELECT ${COLS} FROM records_fts f JOIN records r ON r.rowid = f.rowid ` +
        ...
        for (const row of rows) {
            ...
            const cost = (0, memory_seed_1.estimateTokens)(rec);
    ...
    recent(kind, limit = exports.DEFAULT_LIMIT) {
    ...
    stats() {
        ...
        const current = Number(this.db.prepare(`SELECT COUNT(*) AS n FROM records r WHERE ${CURRENT}`).get()?.['n'] ?? 0);
        const byKind = {};
        for (const row of this.db.prepare(`SELECT r.kind AS k, COUNT(*) AS n FROM records r WHERE ${CURRENT} GROUP BY r.kind ORDER BY r.kind`).all()) {
