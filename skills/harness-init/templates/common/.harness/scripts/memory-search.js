...
exports.splitEntries = splitEntries;
...
function splitEntries(content, opens) {
...
function search(term, stores, root) {
...
function run(argv, root, out) {
    ...
    if (term === '') {
        ...
        out('Default stores:');
        for (const s of STORES.filter((x) => x.primary))
            out(`  ${s.file}`);
        out('With --all, also:');
        for (const s of STORES.filter((x) => !x.primary))
            out(`  ${s.file}`);
    ...
    if (hits.length === 0) {
        ...
        if (!flags.has('--all'))
            out('Re-run with --all to include the stores off the default path.');
        return 1;
    }
