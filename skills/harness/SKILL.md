...
9. **Dispatch `harness-kit:qa-tester`** via Task tool — **with the adversarial verification contract enforced** (see `.harness/playbooks/qa-tester.md`). Output: `06_TEST_REPORT.md` that MUST include `## Adversarial tests` section per acceptance criterion. Update `stage: delivery`.
10. **Write `07_DELIVERY.md`** (PM does this directly): summary + verify_all output + any `## Insight` section if the task surfaced non-obvious project truths. At this delivery boundary the PM also runs the cadenced, non-blocking entropy watch (full mode only) — see `.harness/playbooks/pm-orchestrator.md` → "Entropy watch at delivery". *(referencing line only — the call-sequence and section-format live there, not here)*
