...
- .claude/settings.json (Claude Code binding glue from templates/common/.claude/settings.json.tmpl; wires the four harness hooks — Stop/sync, PreToolUse/guard-rm, UserPromptSubmit/ambient-prompt, SessionStart/ambient-reset — merged in per event if your settings.json already exists, see "Hooks merge" below)
...
- .harness/scripts/harness-sync.sh, .harness/scripts/verify_all.sh, .harness/scripts/baseline.json
- .harness/scripts/guard-rm.sh (cross-platform destructive-command guard; see .harness/rules/75-safety-hook.md)
...
## Hooks merge (.claude/settings.json — all four events, v0.31+)

If .claude/settings.json already exists in this project, each of the four harness hook
events (Stop / PreToolUse / UserPromptSubmit / SessionStart) is merged independently:
  - Event has NO entry → ADD the OS-picked harness entry. No conflict.
  - Event entry already points at the harness script → leave as-is. No conflict.
  - Event entry points at something else → surface as a conflict; log to
    .harness-adopt/CONFLICTS.md; do not modify; ask the user.
...
**Special case — .claude/settings.json (hooks merge, all four events — v0.31+):**

This file is treated differently than other "differs in merge mode" files
because the four harness hook entries are additive and almost always safe to
merge. The four events and their OS-picked commands (substitution table below):
Stop → `{{SYNC_COMMAND}}`, PreToolUse (matcher=="Bash") → `{{GUARD_COMMAND}}`,
UserPromptSubmit → `{{AMBIENT_PROMPT_COMMAND}}`, SessionStart → `{{AMBIENT_RESET_COMMAND}}`.

- If target is absent: copy the template with **all four** command placeholders
  substituted (OS-picked per the table below); done.
- If target exists and is byte-identical to template: skip; done.
- If target exists and differs:
  - Parse existing as JSON.
  - **Per event** (Stop, PreToolUse, UserPromptSubmit, SessionStart):
    - No entry for the event → add the OS-picked harness entry (for PreToolUse, the
      matcher=="Bash" block pointing at `.harness/scripts/guard-rm.*`). Write back
      with stable JSON formatting (2-space indent, preserve key order, preserve the
      root `_*` doc keys).
    - The event already has an entry whose command points at the harness script →
      leave it untouched.
    - The event has an entry pointing at something else → log the conflict to
      `.harness-adopt/CONFLICTS.md` and DO NOT modify that event; surface the
      conflict in the final summary so the user can merge manually.

JSON-merge implementation note: prefer `jq` if
...
1. Run the binding sync (`.harness/scripts/harness-sync.sh`).
...
3. **Hook congruence + settings integrity assertion (mandatory — T-020 / FR-P4, gate C4).**
   Before reporting success in step 7:
   - Read the final `.claude/settings.json`. For every line containing `"command"`,
     extract each script path matching the left-bounded pattern
     `(^|["' =])(\.harness/)?scripts/<name>.(ps1|sh)` (the boundary means a dirname
     merely *ending* in `scripts/`, e.g. `build-scripts/deploy.sh`, is NOT extracted —
     pre-existing user hooks are never flagged). Assert every extracted file exists
     (Glob/Read) and that no `"command"` line carries an unresolved `{{...}}` token.
   - **After any merge-mode settings write, additionally assert** (gate C4): the file
     parses as JSON; `$schema` is exactly
     `https://json.schemastore.org/claude-code-settings.json`; every key inside
     `hooks` is a valid Claude Code hook event name; the root `_*` doc keys
     (`_comment`, `_doc_sync_hook`, `_guard_hook`, `_ambient_hook` — those that
     existed before the write) survived the re-serialize.
   - On any violation: report **flow failure** naming the hook event, the command,
     and the missing path / failed integrity check; fix (re-copy the named script,
     re-substitute the token, or restore from the pre-write state) and re-assert.
     **Do not print the step-7 success summary until this assertion passes.**
