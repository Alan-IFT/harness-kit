...
If .claude/settings.json already exists in this project:
  ...
  - If it HAS a `hooks.PreToolUse` array but no matcher==="Bash" entry pointing at scripts/guard-rm.* → APPEND a new matcher=="Bash" block. No conflict.
...
- If target exists and differs:
  ...
  - If no `hooks.PreToolUse`: insert a fresh `PreToolUse` array with the
    matcher=="Bash" block pointing at `scripts/guard-rm.*`. Write back with
