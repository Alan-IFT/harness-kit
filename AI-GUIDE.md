...
Before declaring any task complete, run `.harness/scripts/verify_all` and confirm all PASS checks are green (34/34; check count grows with releases) — this is the gate, not a rule fragment.
...
- `verify_all.{ps1,sh}` — total verification (34 checks). **Must PASS before declaring done.**
