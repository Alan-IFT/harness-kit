...
- **`.harness/rules/40-locations.md`** (**when looking for "where does X live"**, **or which script does X**): the file-location lookup table, the script index (each script's own header is its contract) and the TypeScript build/ship loop — read this if you'd otherwise guess a path
- **`.harness/rules/60-tool-handoff.md`** (**when switching Claude Code ↔ Copilot or other tools**, **or asking which flow is supported**): which flows are first-class (plugin-native agents + `Task` dispatch; non-Claude tools are not, and why), state lives in files, doc-sync responsibility for non-Claude tools
