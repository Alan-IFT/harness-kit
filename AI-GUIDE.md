...
Before declaring any task complete, run `.harness/scripts/verify_all` and confirm all PASS checks are green (32/32 at v0.21.1; check count grows with releases) — this is the gate, not a rule fragment.
...
- `.harness/scripts/verify_all.{ps1,sh}` — total verification (32 checks at v0.21.1, including I.1-I.5 doc-size WARN guards + F.2 guard-rm wiring + I.6 gap-tolerant retired-claim guard + I.7 ignored-INTERVENE-report guard + D.3 AI-generated 50-*.md sanity + J.1 settings.json schema integrity). **Must PASS before declaring done.**
