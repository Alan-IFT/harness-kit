...
Before declaring any task complete, run `scripts/verify_all` and confirm all PASS checks are green (30/30 at v0.17.4; check count grows with releases) — this is the gate, not a rule fragment.
...
- `scripts/verify_all.{ps1,sh}` — total verification (30 checks at v0.17.4, including I.1-I.5 doc-size WARN guards + F.2 guard-rm wiring + I.6 retired-claim phrase guard + I.7 ignored-INTERVENE-report guard + D.3 AI-generated 50-*.md sanity). **Must PASS before declaring done.**
