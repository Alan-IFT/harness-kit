...
Before declaring any task complete, run `scripts/verify_all` and confirm all PASS checks are green (28/28 at v0.15.1; check count grows with releases) — this is the gate, not a rule fragment.
...
- `scripts/verify_all.{ps1,sh}` — total verification (28 checks at v0.15.1, including I.1-I.5 doc-size WARN guards + F.2 guard-rm wiring + I.6 retired-claim phrase guard). **Must PASS before declaring done.**
