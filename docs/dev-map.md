...
│
├── scripts/
│   ├── verify_all.{ps1,sh}             ← Total verification (30 checks at v0.17.2)
...
│   ├── test-supervisor.{ps1,sh}        ← Supervisor agent + /harness-supervise skill regression (57 assertions PS / 53 Bash-no-python3 at v0.17.2; includes BUG-1 fixed-case Q-1 + BUG-2 column-anchored-slug negative fixtures)
...
| Total verification | `verify_all` | Single source of truth for "is the repo healthy" — runs all 30 checks (at v0.17.2) including both `--check` modes |
