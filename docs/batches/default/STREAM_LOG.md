...
## Run 2026-06-20d — consumer bug fix: resilient hooks (T-12)
- 2026-06-20 · baseline verify_all.sh 32/0/0 (post-v0.43.0). Real consumer report: per-turn `Stop hook error: bash .harness/scripts/harness-sync.sh: No such file or directory`. Root-caused (Stop hook brittle relative-path; not a plugin leak — claude-code-guide confirmed plugin .claude/settings.json isn't loaded). Continuous driver.
- 2026-06-20 · T-12 · dispatching stage-1 requirement-analyst · slug=resilient-hooks · mode=full
- 2026-06-21 · T-12 · **DELIVERED** v0.44.0 (0 rollbacks): RA READY → SA READY (OQ-3 solved — `cd`-anchor + bare token keeps E.4b/D.4b 0-edit verify-only) → Gate APPROVED-WITH-CONDITIONS (caught 2 missed lockstep surfaces: `templates/common` 8-file sync-self lockstep + test-init `test_migrate`) → Dev 32/0/0 (25 files) → CR APPROVED-WITH-NITS → post-CR fidelity fix (3 brittle fixtures → resilient, AC-7 met) → QA PASS (Fixture Z fail-closed mutation probe added; all 4 user-observable behaviors verified incl. guard-rm fail-CLOSED present rc=2/absent rc=127). Consumer Stop-hook error eliminated by design (fail-open + `$CLAUDE_PROJECT_DIR` anchor); guard-rm stays fail-CLOSED; dogfood hooks → gitignored settings.local.json. verify_all.sh 32/0/0 (PS operator-pending). 1 insight harvested (bash 5.2 patsub_replacement `&` corruption), 1 rotated. 07: docs/features/_archived/resilient-hooks/07_DELIVERY.md
