...
.harness/`). Just run `harness-sync` and re-verify. A Stop hook runs `harness-sync` automatically
at session end, so this rarely bites you in Claude Code. A project generated by
`/harness-init` ships that hook in its committed `.claude/settings.json`;
harness-kit's own checkout keeps its wiring machine-local — run `/harness-status`
§0 "Effective hook source" if you need to know which file a given project loads it
from.
