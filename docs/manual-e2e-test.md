...
1. **Skill discovery** — does Claude Code actually load the twelve skills?
...
**Expected**: prints "Would copy" for all 12 skills (harness, harness-init,
...
**Expected**: prints "Installed" for all 12 skills. After completion, list them:
...
# harness-init, harness-intervene, harness-plan, harness-status, harness-stream, harness-supervise, harness-verify
```

### A.3 Claude Code sees them

Open Claude Code in any folder. Type `/help` or look at the slash command picker.
**Expected**: the twelve `/harness-*` commands appear (`/harness`, `/harness-init`,
