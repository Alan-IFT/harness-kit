...
A consumer project reported a per-turn `Stop hook error: bash .harness/scripts/harness-sync.sh: No such file or directory` — a wired hook pointing at a script that does not exist. Root cause class: every flow rewired `.claude/settings.json` hook paths **unconditionally**, without verifying the target script actually landed. This release makes the settings rewire presence-gated in the deterministic helpers, adds a terminal hook↔script congruence assertion to all four flows, and gives already-broken projects a first-class diagnose (`/harness-status`) + repair (`/harness-upgrade`) path.

- **`migrate-scripts-layout.{ps1,sh}`** (template + dogfood mirror): per-variant presence-gated settings rewire (a rewire can no longer point a hook at a file that never landed), move verification (`MOVE-FAILED` line for a silently failed `git mv`/`mv`), and a terminal congruence scan — any wired command referencing a missing script prints `CONGRUENCE-FAIL <command> -> missing <path>` plus the `/harness-upgrade` hint and the run exits with new code **4** (healthy/idempotent runs still exit 0). In apply mode the scan re-reads the written settings from disk, so a settings write that never landed (read-only file, disk full) is caught too — never a silent exit 0.
- **`upgrade-project.{ps1,sh}`** (template + dogfood mirror): S2 refresh set widened by the ambient hook pair (`ambient-prompt.{ps1,sh}`, `ambient-reset.{ps1,sh}`); template-absent refresh entries are no longer skipped silently (`GAP|template-missing|absent|<path>` / NOOP-retained); each S2 copy is verified (`CONFLICT|refresh` on a copy that did not land); S3 prefix rewires are presence-gated per variant; **new S3 first pass repairs a wired literal placeholder token** (unsubstituted RC-3 improvisation damage) to the OS-picked command — gated on the target being landable, so a malformed token never becomes a dangling path (`REWIRE-PLACEHOLDER` record); terminal congruence scan emits `CONFLICT|congruence|...` and exits **4** (overrides 2/3; all records stay on stdout). Healthy projects keep exiting 0 with zero writes.
- **Ambient hooks OS-picked at init**: `settings.json.tmpl`'s two hard-coded `pwsh` ambient commands are now `{{AMBIENT_PROMPT_COMMAND}}` / `{{AMBIENT_RESET_COMMAND}}`, OS-picked with the same rule as `{{SYNC_COMMAND}}` (Windows → `pwsh -NoProfile`, elsewhere → `bash`) — kills the same wired-but-unrunnable class on macOS/Linux. Placeholder count 7 → 9 (documented in `harness-init`/`harness-adopt` SKILL tables; whitelisted in dogfood D.2, both shells).
- **`/harness-init`**: new mandatory step 10b — terminal hook congruence assertion (every `"command"`-line script path exists, no unresolved `{{...}}` token) before the success summary may print.
- **`/harness-adopt`**: substitution table gains the four hook-command placeholders (the Stop-hook command is never improvised again); the settings special-case is generalized from PreToolUse-only to a **four-event hooks merge** (Stop / PreToolUse / UserPromptSubmit / SessionStart, per-event add/leave/conflict); terminal congruence + merge-mode settings-integrity assertion (JSON parses, canonical `$schema`, valid hook event keys, `_*` doc keys survive).
- **`/harness-upgrade`**: documents exit `4` (incl. co-occurring exit-2/3 record processing and the dry-run leg), the `REWIRE-PLACEHOLDER` record, the `GAP|template-missing` / `CONFLICT|refresh` / `CONFLICT|congruence` kinds, the repair framing in "When to invoke", and a `Congruence:` + `Repaired:` line in the final report.
- **`/harness-status`**: new §3c hook↔script congruence report for all four events (`ok` / `not wired` / `DANGLING` / `MALFORMED` + interpreter-availability WARN with the variant-swap instruction); the stale "All 7 agents" / "Supervisor" asset rows are replaced by the v0.30 plugin-provided note (required-asset count now a consistent 14).
- **Type `verify_all` templates (all six)**: the stale v0.2x agents FAIL row (which failed every healthy v0.30 project) is replaced with "Agents layout v0.30+ (`.harness/agents/` = partition `dev-*` only)" — PASS when absent or dev-only, WARN listing legacy framework copies; new **`E.4b`/`D.4b` hook-congruence row** FAILs when a wired hook command references a missing script or carries an unresolved placeholder (fix line: run `/harness-upgrade`). The path matcher in every consumer is **left-boundary-guarded** (quote/space/`=`/line start), so a custom hook in a dirname merely ending in `scripts/` (e.g. `build-scripts/deploy.sh`) is never flagged. Rows live outside the `HARNESS:B-CUSTOM` markers (upgrade splice untouched).
- **Regression drivers**: `test-harness-upgrade.{ps1,sh}` gains fixtures for dangling repair + idempotence, incongruent end state (crafted template root), literal-placeholder repair (dry-run + apply + re-run) and the gated-off token, the migrate RC-1/healthy pair, a read-only-settings write-failure probe (self-disabling where the read-only bit is unenforceable, e.g. root), and the C1 custom-hook false-positive guard (bash 37→79, ps 38→80 assertions); `test-init.{ps1,sh}` substitutes the new ambient placeholders and asserts the generated settings' hook congruence, the OS-picked ambient commands, the new verify_all rows, and a delete-the-script mutation probe (ps 287→308); `test-real-project.{ps1,sh}` runs the generated type `verify_all` on healthy single-dev and dangling states (82→90).
- Version 0.30.1 → 0.31.0 (plugin.json, marketplace.json, both README badges). Skill count stays **15**; `verify_all` stays **32** checks; no I.6 banned/exempt-list change.
...
- **`/harness-init` is plugin-native by default**: the generic framework agents are no longer copied into a new project; only partition projects get their `dev-*` agents locally (and `harness-sync` syncs only those to `.claude/agents/`). harness-kit is now openly **Claude-Code-native** for the framework agents (a tool-agnostic/offline materialization for non-Claude tools is not implemented).
...
- **Docs**: `AI-GUIDE.md`, `README.md`, `README.zh-CN.md`, `.harness/rules/40-locations.md`, and `docs/dev-map.md` updated — framework agents are plugin-provided (`harness-kit:<name>`); only partition `dev-*` live in `.harness/agents/`; positioning set to openly Claude-Code-native.
...
- **`install.ps1` + `install.sh`** now **derive** the skill list from the `skills/` tree (each `skills/<name>/` with a `SKILL.md`) instead of a hardcoded array — a newly-added skill is auto-included and can never be silently dropped again. This also fixes a pre-existing gap (T-018 OQ-1): the old hardcoded list had omitted `harness-upgrade` (v0.23) and `harness-language` (v0.25), so script-path installs were shipping only 13 skills; **all 15 now install** (verified via `install.sh --dry-run`). `verify_all` C.1/G.1/G.2 skill enumeration + labels go 14 → 15.
