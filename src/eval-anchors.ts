...
export interface Anchor {
  /** The evals document the citation appears in. */
  source: string;
  /** 1-based line of the citation inside that document. */
  sourceLine: number;
  /** The cited project-relative path. */
  target: string;
  /** Line numbers cited, if any. */
  lines: number[];
  /** The citation as written. */
  raw: string;
}

export interface Stale extends Anchor {
...
function isCitation(target: string, suffix: string): boolean {
  return target.includes('/') || suffix !== '';
}

export function parseAnchors(source: string, content: string): Anchor[] {
  const out: Anchor[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const m of line.matchAll(ANCHOR_RE)) {
      ...
      if (!isCitation(target, suffix)) continue;
 ...
 * are reported.
 */
export function resolveTarget(target: string, root: string): string | null {
...
export function checkAnchors(anchors: readonly Anchor[], root: string): Stale[] {
  const stale: Stale[] = [];
  ...
  for (const a of anchors) {
    ...
    const over = a.lines.filter((n) => n > (count as number));
    if (over.length > 0) {
      stale.push({
        ...a,
...
export function auditEvals(root: string): { anchors: Anchor[]; stale: Stale[] } {
...
export function run(argv: readonly string[], root: string, out: (s: string) => void): number {
  let result: { anchors: Anchor[]; stale: Stale[] };
  try {
