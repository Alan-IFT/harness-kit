...
import { SEED_KINDS, SeedKind, SeedRecord, harvest } from './memory-seed';
import {
  DEFAULT_LIMIT,
...
export const ROLE_QUESTIONS: ReadonlyArray<{ role: string; question: string; kinds: SeedKind[] }> = [
...
function parseArgs(argv: readonly string[]): { cmd: string; flags: Set<string>; opts: Map<string, string>; rest: string[] } {
...
function fmt(rec: SeedRecord, out: Out): void {
  out(`  [${rec.kind}] ${rec.title}`);
  out(`      seq ${rec.seq}  ${rec.date === '' ? 'no-date' : rec.date}  ${rec.tags.join(' ')}`);
  if (rec.evidence.length > 0) out(`      evidence: ${rec.evidence.join('; ')}`);
  if (rec.supersedes !== undefined) out(`      supersedes: ${rec.supersedes}`);
}

function openSink(root: string, out: Out): SqliteSink | null {
  try {
    return new SqliteSink(resolveDbPath(root));
  } catch (err) {
    if (err instanceof MemoryDbPathError) {
...
export function run(argv: readonly string[], root: string, out: Out): number {
  ...
  const kinds = (opts.get('--kinds') ?? opts.get('--kind') ?? '')
    .split(',')
    .map((k) => k.trim())
    .filter((k): k is SeedKind => (SEED_KINDS as readonly string[]).includes(k));

  if (cmd === 'seed') {
    const from = path.resolve(root, opts.get('--from') ?? '.');
    const result = harvest(from);
    const byKind: Record<string, number> = {};
    for (const r of result.records) byKind[r.kind] = (byKind[r.kind] ?? 0) + 1;

    out(`Harvested ${result.records.length} records from ${path.relative(root, from) || '.'}`);
    for (const s of result.sources) out(`  ${s.source.padEnd(46)} ${String(s.records).padStart(4)} records  ${s.unparsed} unparsed`);
    out('');
    for (const k of SEED_KINDS) out(`  ${k.padEnd(10)} ${byKind[k] ?? 0}`);
    ...
    if (sink === null) return 2;
    let res;
    let st;
    let pruned = 0;
    try {
      res = sink.upsert(result.records);
      if (!flags.has('--no-prune')) pruned = sink.prune(new Set(result.records.map((r) => r.id)));
      st = sink.stats();
    } finally {
  ...
  if (cmd === 'stats') {
    ...
    try {
      ...
      for (const k of SEED_KINDS) out(`  ${k.padEnd(10)} ${st.byKind[k] ?? 0}`);
  ...
  if (cmd === 'search') {
    ...
    if (sink === null) return 2;
    try {
      const opt: { limit: number; tokenBudget: number; kinds?: SeedKind[] } = {
        limit: num('--limit', DEFAULT_LIMIT),
        tokenBudget: num('--budget', DEFAULT_TOKEN_BUDGET),
      };
      if (kinds.length > 0) opt.kinds = kinds;
      ...
      for (const r of res.records) fmt(r, out);
      return res.records.length === 0 ? 1 : 0;
    } finally {
      sink.close();
    }
  ...
  if (cmd === 'recent') {
    ...
    if (sink === null) return 2;
    try {
      const recs = sink.recent(kinds[0] ?? null, num('--limit', DEFAULT_LIMIT));
      if (json) {
        out(JSON.stringify(recs, null, 2));
        return recs.length === 0 ? 1 : 0;
      }
      for (const r of recs) fmt(r, out);
      return recs.length === 0 ? 1 : 0;
    } finally {
      sink.close();
    }
  ...
  if (cmd === 'roles') {
    ...
    let empty = 0;
    try {
      for (const rq of ROLE_QUESTIONS) {
        ...
        if (top === undefined) {
          ...
          for (const r of res.records) out(`  → [${r.kind}] ${r.title.slice(0, 150)}`);
        }
        out('');
      }
    } finally {
