...
exports.CATEGORIES = exports.CONTROL_SET = void 0;
exports.splitRow = splitRow;
exports.parseCases = parseCases;
exports.backticked = backticked;
exports.isCheckableToken = isCheckableToken;
exports.anchorPaths = anchorPaths;
exports.checkClaims = checkClaims;
exports.loadCases = loadCases;
exports.run = run;
const fs = require("node:fs");
const path = require("node:path");
exports.CONTROL_SET = 'evals/retrieval-eval.md';
exports.CATEGORIES = ['CODE', 'MEM', 'RULE', 'STATE'];
// ---------------------------------------------------------------------------- parse
/**
...
function parseCases(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (cells.length < 4)
            continue;
        const m = ID_RE.exec(cells[0]);
        if (m === null)
            continue;
        const category = CATEGORY_OF[m[1]];
 ...
 */
function isCheckableToken(token) {
    if (/\s/.test(token))
        return false;
    if (token.includes('/'))
        return false;
 ...
 */
function expandStar(pattern, root) {
    ...
    const re = new RegExp(`^${parts[at].split('*').map((s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('.*')}$`);
    let entries;
    try {
 ...
 * spans them.
 */
function anchorPaths(anchor, root) {
    const out = [];
...
function checkClaims(cases, root) {
    const findings = [];
    const cache = new Map();
    const read = (rel) => {
    ...
    for (const c of cases) {
        ...
        if (paths.length === 0) {
            ...
            findings.push({ id: c.id, line: c.line, token: tokens[0], kind: 'anchor-unreadable', anchorPath: c.anchor });
            continue;
        }
...
function loadCases(root) {
    return parseCases(fs.readFileSync(path.join(root, exports.CONTROL_SET), 'utf8'));
}
// ---------------------------------------------------------------------------- cli
function run(argv, root, out) {
    ...
    try {
        ...
        out(`eval-set: cannot read ${exports.CONTROL_SET}`);
        return 2;
    }
    if (flags.has('--list')) {
        const only = valueOf('--category')?.toUpperCase();
        for (const c of cases) {
            if (only !== undefined && c.category !== only)
                continue;
    ...
    if (flags.has('--check')) {
        ...
        if (findings.length === 0) {
            const per = exports.CATEGORIES.map((k) => `${k} ${cases.filter((c) => c.category === k).length}`).join(', ');
            out(`${cases.length} control-set cases (${per}); every backticked claim resolves in its anchor.`);
            return 0;
        }
        for (const f of findings) {
            out(`${exports.CONTROL_SET}:${f.line}: ${f.id}: ${f.kind}: \`${f.token}\` not in ${f.anchorPath}`);
        }
        return 1;
    }
