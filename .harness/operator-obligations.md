...
### V2TS-1 — the PowerShell launchers and the re-anchored FC-4 rows, on Windows

- Id: V2TS-1
- Action: `hook-spec.ps1`, `guard-rm.ps1` and `install-hooks.ps1` are no longer implementations. Each is now a two-line launcher (`& node (Join-Path $PSScriptRoot "<name>.js") @args; exit $LASTEXITCODE`) over the compiled TypeScript, in both the dogfood tree and `templates/common`. **None of the six has been executed** — this host has no `pwsh`. Four things to confirm. First, **stdin passes through**: `guard-rm` reads its tool-input JSON on stdin, and a launcher that swallows or re-encodes it disarms the guard silently. Second, **the exit status propagates**: `$LASTEXITCODE` must reach the caller unchanged, because 2 means BLOCK and anything that flattens it to 0 is fail-OPEN. Third, **a missing `node` must leave a non-zero status**, which is the fail-CLOSED direction; do not add a fallback. Fourth, `test-init.ps1`'s two FC-4 rows were re-anchored from mutating `hook-spec.ps1` to mutating `hook-spec.js`, because the installer now imports the compiled module rather than shelling out — the old stub would never be read and the row would pass against an unreachable branch. The Windows path separator in the generated `require(...)` is normalised with `-replace '\\', '/'`; confirm the stub actually loads rather than throwing a module-not-found that would exit 4 by the wrong route and leave the row green while proving nothing.
- Artifacts: `.harness/scripts/{hook-spec,guard-rm,install-hooks}.ps1`, the same three under `skills/harness-init/templates/common/.harness/scripts/`, `.harness/scripts/test-init.ps1` rows 7 and 8, `.harness/scripts/sync-self.ps1` Mapping 10
- Pass observable: `pwsh -File .harness/scripts/test-guard-rm.ps1` reports 87/0 driving the launcher; `pwsh -File .harness/scripts/test-init.ps1` is green including both FC-4 rows, each matching its own diagnostic (`expected 4 ids, got 3` and `expected 4 DISTINCT hook events, got 1`) rather than merely exiting 4; `pwsh -File .harness/scripts/verify_all.ps1` reports 32/0/0 exit 0; and `pwsh -File .harness/scripts/sync-self.ps1 -Check` reports in-sync with the three `.js` entries of Mapping 10 present
- Security: **yes** — `guard-rm` is the fail-CLOSED destructive-command guard, and two of the four confirmations above (stdin passthrough, exit-status propagation) are the difference between a live guard and a silently disarmed one
- Origin: v2 migration, shell-to-TypeScript stages 1-3. The bash launchers were proved live: the 87-case corpus drives the TypeScript implementation through `guard-rm.sh` and passes 87/0, `test-init.sh` passes 391/0 with both re-anchored FC-4 rows, and the session's own `PreToolUse` hook has been running through the launcher since it was written.
- Last discharged: never
