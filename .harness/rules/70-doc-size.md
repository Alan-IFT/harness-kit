...
|---|---|---|---|
| `AI-GUIDE.md` | 200 lines **and** 20 KB | Always-on entry; reread every task. At ~147 bytes per line the line cap alone admits ~29 KB, and what a session pays for is bytes, not line count. | Trim to index-only; move prose to `docs/concepts.md` or a rule fragment |
...
| `agents/*.md` (each) | 300 lines **and** 24 KB | A subagent's system prompt IS this file's body, so it is paid on every dispatch and paid in bytes. The line cap cannot see line length — a 150-line contract of 200-character paragraphs passes it while costing more than a 300-line one. This is the path `verify_all` I.3 checks — in THIS repo the framework agents are plugin-native at `agents/`. Partition `dev-*` agents live at `.harness/agents/` (empty here; that is where a generated project's agents sit) and carry the same cap. | Refactor responsibilities, or move stable doc to `docs/` |
| `.harness/insight-index.md` | 30 insight entries **and** 24 KB | Loaded at every task start by PM. Rotation holds the entry count at 30 but bounds nothing else — entries here average ~700 bytes, so the count cap admits unbounded size. | `.harness/scripts/archive-task` auto-rotates oldest to `docs/features/_archived/insight-history.md`; if the byte arm fires, tighten existing entries rather than adding another |
| `docs/tasks.md` | 300 lines **and** 24 KB | Loaded at every task start by PM. The byte cap is the load-bearing one: a row is one physical line however much prose its Outcome cell carries, so the line cap alone is vacuous (the file reached 57 KB at 65 lines and PASSed). | Compact Outcome cells to one clause — the task's own `07_DELIVERY.md` holds the record and the Doc folder column already addresses it. Then move oldest Completed rows to `docs/tasks-archive.md` |
| Per-task `PM_LOG.md` | 500 lines | PM rereads for resume; downstream agents read for context. | "Compaction" pattern below |
| Per-task stage doc (`0[1-7]_*.md`) | 500 lines **and** 48 KB each | Read by the next stage's agent, and paid in bytes. 48 KB is the byte equivalent of the line cap that was already here: across 302 archived stage documents the median rate is 94 bytes per line, so 500 lines already intended ~46 KB. Stating it in bytes changes no intent — it stops a document passing at 147 B/line, which `03_GATE_REVIEW` for `guard-cmd-chain` did at **68.3 KB in 475 lines**. The line arm alone had no checker at all until `verify_all` I.8. | "Reference, don't paste" pattern below; route the excess to the `0N_RATIONALE.md` sibling per the boundary rule |
...
(see `.harness/playbooks/pm-orchestrator.md` step 10).
