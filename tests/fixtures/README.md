...
3. Run the project's own `scripts/harness-sync` to populate `.claude/agents/` and `.claude/skills/` from `.harness/` (since v0.10, `CLAUDE.md` is a one-time init-written stub, not regenerated by `harness-sync`).
4. Assert that:
